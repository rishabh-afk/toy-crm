...
const LedgerTransactionsMModal = ({ data }: LedgerTransactionsMModalProps) => {
  ...
  const [activeTab, setActiveTab] = useState<"Payment" | "Receiving">(
    data?.types || "Payment"
  );

  const fetchTransaction = async (type: "Payment" | "Receiving") => {
    const endpoint = endpoints[type]?.fetchAll;
    const url = `api/${type === "Payment" ? "payment" : "receiving"}/total/${
      data?.id
    }`;
  ...
  return (
    <div className="">
      {/* Tab Header */}
      <span className="text-2xl absolute top-5 font-semibold">
        Ledger Transactions
      </span>
      <div className="my-5">
            ...
            <thead className="bg-gradient-to-r from-blue-50 to-blue-100 text-blue-700 font-semibold">
              <tr className="whitespace-nowrap text-left">
                <th className="px-6 py-3">
                  {activeTab === "Payment" ? "Payment" : "Receiving"} No
                </th>
                <th className="px-6 py-3">Ledger Name</th>
                <th className="px-6 py-3">
                  {activeTab === "Payment" ? "Payment" : "Receiving"} Type
                </th>
                <th className="px-6 py-3">
                  {activeTab === "Payment" ? "Payment" : "Receiving"} Date
                </th>
              ...
              transaction.map((item: any, idx: number) => (
                ...
                >
                  <td className="px-6 py-3">
                    {item[
                      activeTab === "Payment" ? "paymentNo" : "receivingNo"
                    ] ?? "-"}
                  </td>
                  ...
                  <td className="px-6 py-3">
                    {item[
                      activeTab === "Payment" ? "paymentType" : "receivingType"
                    ] || "-"}
                  </td>
                  <td className="px-6 py-3">
                    {item[
                      activeTab === "Payment" ? "paymentDate" : "receivingDate"
                    ]
                      ? new Date(
                          item[
                            activeTab === "Payment"
      ...
      {transaction.length === 0 && (
        <div className="flex flex-col items-center justify-center py-10 text-gray-500">
          ...
          <p className="text-sm text-center max-w-xs mt-1">
            We couldn&apos;t find any transactions linked to this record. Once
            they&apos;re available, you&apos;ll see them here.
          </p>
        </div>
      )}
